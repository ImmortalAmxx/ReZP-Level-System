...
@CBasePlayer_UpdateClientData_Post(id)
{
	...
	add_formatex("^n[%l: %d]", "RZ_ARMOR", floatround(get_entvar(id, var_armorvalue)));
	...
	add_formatex("^n[%l: %d]", "RZ_LEVEL", rz_get_user_level(id));
	add_formatex("^n[%l: %d\%d]", "RZ_EXP", rz_get_user_exp(id), rz_get_user_next_exp(id));
